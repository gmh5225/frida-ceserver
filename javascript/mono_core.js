...
var mono_method_get_flags = new NativeFunction(mono_method_get_namePtr, 'uint', ['pointer', 'int']);
...
function EnumMethodsInClass(_class) {
  ...
  while (true) {
    ...
    if (parseInt(method) != 0) {
      ...
      WriteWord(name.length);
      WriteUtf8String(name);
      var flags = mono_method_get_flags(method, 0);
      WriteDword(flags);
    } else {
      break;
    }
